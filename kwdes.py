...
def base64_encrypt(id, format):
    msg = "corp=kuwo&p2p=1&type=convert_url2&format="+ format + "&rid=" + id
    b1 = encrypt(msg)
    b2 = bytearray(b1)
    s = base64.encodebytes(b2)
    return "http://mobi.kuwo.cn/mobi.s?f=kuwo&q=" + s.replace(b'\n', b'').decode()

print(base64_encrypt("6468891", "ape|flac|mp3|aac"))
